...
Gene2Vec has supplied a trained embedding matrix in `embedding matrix.csv`. The training dataset was chosen as the whole RefSeq genomes, the NCBI accession number of which.can be found in `NCBI accession numbers of the whole Refseq genomes.xlsx`. If you would like to train the embedding matrix by youself, jsut run `embedding.py`.
